 ...
 */
class Tests_Dirlist extends WP_Filesystem_Direct_UnitTestCase {
	...
	/**
	 * Tests that `WP_Filesystem_Direct::dirlist()` returns false
	 * when `dir()` fails.
	 */
	public function test_should_return_false_when_dir_fails() {
		global $wp_filesystem;

		$path = self::$file_structure['test_dir']['path'] . 'testdir';

		// Set up mock filesystem.
		$filesystem_mock = $this->getMockBuilder( 'WP_Filesystem_Direct' )
								->setConstructorArgs( array( null ) )
								// Note: setMethods() is deprecated in PHPUnit 9, but still supported.
								->setMethods( array( 'is_dir', 'is_readable' ) )
								->getMock();

		$filesystem_mock->expects( $this->once() )->method( 'is_dir' )->willReturn( true );
		$filesystem_mock->expects( $this->once() )->method( 'is_readable' )->willReturn( true );

		$wp_filesystem_backup = $wp_filesystem;
		$wp_filesystem        = $filesystem_mock;

		$actual        = $filesystem_mock->dirlist( $path );
