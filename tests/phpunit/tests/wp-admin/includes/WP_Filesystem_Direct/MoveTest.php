 ...
 */
class Tests_Move extends WP_Filesystem_Direct_UnitTestCase {
	...
	/**
	 * Tests that `WP_Filesystem_Direct::move()` returns false
	 * when the source and destination are the same.
	 */
	public function test_should_return_false_when_source_and_destination_are_the_same() {
		$source = self::$file_structure['visible_file']['path'];
		$actual = self::$filesystem->move( $source, $source, true );

		$this->assertFalse( $actual, 'False was not returned.' );
		$this->assertFileExists( $source, 'The source file was deleted. This test has attempted to re-create it.' );
	}
